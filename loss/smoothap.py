...
class LossFunction(nn.Module):
    ...
    def __init__(self, nPerSpeaker, nOut, batch_size, anneal=1, **kwargs):
        """
        Parameters
        ----------
        anneal : float
        ...
        #assert(batch_size % nPerSpeaker==0)

        self.anneal = anneal
        self.batch_size = batch_size * nPerSpeaker
        self.num_id = nPerSpeaker
        self.feat_dims = nOut

        print('Initialised SmoothAP')

    def forward(self, x, label=None):
        """Forward pass for all input predictions: preds - (batch_size x feat_dims) """
        preds    = x.reshape(-1,x.size()[-1])
        ...
        nloss = 1 - ap
        prec1 = accuracy(sim_pos_rk.view(-1,int(self.batch_size / self.num_id)).detach(), torch.repeat_interleave(label,repeats=self.num_id,dim=0).detach(), topk=(1,))[0]
